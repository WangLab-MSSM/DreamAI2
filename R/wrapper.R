...
#' method=c("KNN","MissForest","ADMIN","Birnn","SpectroFM","RegImpute","MICE"),out=c("Ensemble"),
#' SamplesPerBatch=1,n.bag=2,save.out=TRUE,path="dir_of_bagging_results",ProcessNum=1)
#' final.out<-bag.summary(method=c("Ensemble"),nNodes=2,path="dir_of_bagging_results")
#' final.out$score
#' final.out$imputed_data
#' }
bag.summary<-function(method=c("KNN", "MissForest", "ADMIN", "Birnn", "SpectroFM", "RegImpute","MICE","Ensemble","Ensemble.Fast"),
                      nNodes=2, path=NULL)
{
  node.temp = 1
  bag.output = readRDS(paste(path,"bag_imputed_",sprintf("%03d",node.temp),".rds",sep=""))
  ...
  for(i in 1:nNodes)
  {
    bag.output = readRDS(paste(path,"bag_imputed_",sprintf("%03d",i),".rds",sep=""))
    ...
    if("RegImpute" %in% method){
      RegImpute.out[[i]]<-bag.output$impute$RegImpute
    }else{
      sink("NULL")
    ...
    if("MICE" %in% method){
      MICE.out[[i]]<-bag.output$impute$MICE
    }else{
      sink("NULL")
  ...
  summary.out.all.agg = stats::aggregate(cbind(true,imputed)~gene ,data = summary.out.all,function(x){x});

  get.score = function(x)
  {
    dt = x$true;
    di = x$imputed;
    cor.temp = stats::cor(dt,di,use = 'pairwise.complete.obs',method = 'spearman');
  ...
  if("RegImpute" %in% method){
  d.impute.RegImpute<-matrix(0,nrow(RegImpute.out[[1]]),ncol(RegImpute.out[[1]]))
  }
  ...
  for(i in 1:nNodes)
  {
    ...
    if("RegImpute" %in% method){
    d.impute.RegImpute<-d.impute.RegImpute+n.bag.out[[i]]*RegImpute.out[[i]]
    }
  ...
  if("RegImpute" %in% method){
  d.impute.RegImpute.final<-d.impute.RegImpute/n.bag.tot
  ...
  if("MICE" %in% method){
  d.impute.MICE.final<-d.impute.MICE/n.bag.tot
