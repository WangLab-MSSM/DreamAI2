...
#' @param mice.m Number of multiple imputations in MICE. The default is m=1.
#' @param mice.method Specifying the imputation method to be used for each column in MICE. The default is 'pmm'.
#' @param mice.maxit A scalar giving the number of iterations in MICE. The default is 20.
...
DreamAI2<-function(data,k=10,maxiter_MF = 10, ntree = 100,maxnodes = NULL,maxiter_ADMIN=30,tol=10^(-2),gamma_ADMIN=NA,gamma=50,
                   ...
                   m.mice = 1, method.mice = 'pmm', maxit.mice = 20,
                   method=c("KNN","MissForest","ADMIN","Birnn","SpectroFM","RegImpute","MICE"),out=c("Ensemble.Fast"))
{
  ...
  if("MICE" %in% method)
  {
    name = "MICE"
    sink("NULL")
    d.impute.MICE = impute.mice(data = as.matrix(data),k = k)
    # ensemble<-ensemble+d.impute.knn
    sink()
    # print(paste("Method",method.idx,"complete"))
    # method.idx<-method.idx+1
  ...
  df.impute.Ensemble.Fast = reduce('+',imputed_matrix[setdiff(names(impute_matrix),"MissForest")])/length(setdiff(names(impute_matrix),"MissForest"))
